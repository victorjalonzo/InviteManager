...
interface IOptions {
    ...
    roleRewardName: string
}

export class InviteRewardCard {
  static generate = async (options: IOptions): Promise<AttachmentBuilder> => {
    // Canvas Dimensions
    ...
    let {currentInvites, invitesRequired} = options
  
    if (currentInvites > invitesRequired) currentInvites = invitesRequired
    ...
    ? `${options.roleRewardName} role already earned.`
    : `Earn ${options.roleRewardName} role as reward.`;
